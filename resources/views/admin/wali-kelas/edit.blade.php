...
@section('title', 'Edit Wali Kelas - Admin')
            ...
            <div class="col-lg-10 dashboard-content">
                <div class="mb-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="fw-bold">Edit Wali Kelas</h1>
                            <p class="text-muted">Edit data wali kelas</p>
                        </div>
                ...
                <div class="card shadow-sm border-0">
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.wali-kelas.update', $waliKelas->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="semester_id" class="form-control @error('semester_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($semester as $s)
                                            <option value="{{ $s->id }}" {{ old('semester_id', $waliKelas->semester_id) == $s->id ? 'selected' : '' }}>{{ $s->tahun_ajaran }} - {{ $s->semester }}</option>
                                        @endforeach
                                    </select>
                                    @error('semester_id')
